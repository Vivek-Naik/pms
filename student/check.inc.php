...
<body>
  ...
  <div class="content" style="margin-top: 10px; margin-left: 30px;">
    ...
    if (isset($_POST['check'])) {
      $company = mysqli_real_escape_string($conn, $_POST['company']); // Sanitize user input
      $user = $_SESSION['username'];

      // Check if the student has already applied
      $checkAppliedQuery = "SELECT * FROM applied WHERE name = '$user' AND company = '$company'";
      $checkAppliedResult = mysqli_query($conn, $checkAppliedQuery);

      if (mysqli_num_rows($checkAppliedResult) > 0) {
        // Student has already applied
        ?>
        <h5>You have already applied for <b><?php echo $company; ?></b></h5>
  <h3>Please update the status</h3>
        <br><br>
        <p class="lead">Thank you for applying!</p>
        <button class="btn btn-disabled" style="width: 150px; color: white;
        font-weight: bold; background: linear-gradient(to left, #F55197, #EE891A);" disabled>Already Applied</button>
        <?php
      } else {
        // Student has not applied, display the eligibility check and Apply button
              ...
              while ($row1 = mysqli_fetch_assoc($res1)) {
                ...
                if ($studperc >= $compperc) {
                  if ($studperc > 85) {
                    ...
                    <a href="php/apply.inc.php?comp=<?php echo $company; ?>" class="btn" style="width: 150px; color: white;
                    font-weight
                    : bold; background: linear-gradient(to left, #F55197, #EE891A);" name="apply">Apply</a>    
                    <?php
                  } else {
                    ?>  
